...
let urlParams = new URLSearchParams(window.location.search);
    let channelName2 = urlParams.get('channel_name');


fetch(`http://oreumi.appspot.com/channel/getChannelVideo?video_channel=${channelName2}`)
...
async function getVideo(videoList) {
    for (let i = 0; i < videoList.length; i++) {
        ...
        search_video2[i] = data
    }
}

function search() {
    let text = document.getElementsByClassName("ch_search_box")[0].value;
    let value = search_video2.filter((video) => video.video_title.toLowerCase().includes(text.toLowerCase()));
...
// click 시 검색
document.getElementsByClassName("ch_search_box_icon")[0].addEventListener("click", () => {
        search();
    
}); // 혹은 "click" 이벤트를 "input" 이벤트로 변경할 수 있습니다.

// enter 시 검색
document.getElementsByClassName("ch_search_box")[0].addEventListener("keypress", function (e) {
    if (e.keyCode === 13) {
        search();
    }
});
